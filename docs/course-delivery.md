...
- 调度以遗忘曲线为参考：根据间隔、上次复习后实际经过天数计算记忆保持估计，并保存单词的易度、当前间隔、遗忘次数和上次复习日期。
- Again 重置为 1 天；Hard 以较小倍率增长；Good 以易度为基础增长；Easy 使用更大倍率。若单词逾期后仍答对，后续间隔会获得额外增长。
...
| T06 | Again | 重置为 1 天后并降低易度 | 待最终手动验收 |
| T07 | Good | 根据易度与保持估计动态增长 | 待最终手动验收 |
| T08 | 完成复习 | 写入复习日志 | 已通过 |
| T09 | 重启程序 | 数据仍存在 | 已通过 |
