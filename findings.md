...
| 复习调度采用遗忘曲线参考的自适应间隔 | 根据实际记忆表现调整间隔，比固定序列更符合用户目标 |
...
## 自适应复习调度
- 每个单词持久化保存 `interval_days`、`ease_factor`、`lapse_count` 和 `last_review_date`。原数据库会在启动时自动添加这些字段，已有单词不会丢失。
- 调度采用指数遗忘近似 `R = exp(-elapsedDays / stability)` 估计当前保持率；若用户在更长间隔后仍选择 Good / Easy，会增加下一间隔。
- Again 重置为 1 天并降低易度、累计遗忘次数；Hard 低倍率增长；Good 按易度增长；Easy 用更高倍率增长。易度限制在 1.3 到 3.0，避免长期极端化。
